...
public class IdentifierTests
{
    [Fact] public void ProviderPath_is_normalised_to_kind_slash_token()
        => Assert.Equal("gpu/gpu-nvidia-0", HardwareId.FromProviderPath(HardwareKind.Gpu, "/gpu-nvidia/0").Value);
    ...
    {
        var hw = HardwareId.FromProviderPath(HardwareKind.Gpu, "/gpu-nvidia/0");
        var s = SensorId.Create(hw, "temperature/2");
        Assert.Equal("gpu/gpu-nvidia-0#temperature/2", s.Value);
